...
export class VirgilPythiaCrypto implements IPythiaCrypto {
  computeTransformationKeyPair(options: {
    ...
    const myTransformationKeyId = dataToUint8Array(
      options.transformationKeyId,
      "base64"
    );
    const myPythiaSecret = dataToUint8Array(options.pythiaSecret, "base64");
    const myPythiaScopeSecret = dataToUint8Array(
      options.pythiaScopeSecret,
      "base64"
    );
    const {
  ...
  transform(options: {
    ...
    const { Pythia } = getPythiaModules();
    const myBlindedPassword = dataToUint8Array(
      options.blindedPassword,
      "base64"
    );
    const myTweak = dataToUint8Array(options.tweak, "base64");
    const myTransformationPrivateKey = dataToUint8Array(
      options.transformationPrivateKey,
      "base64"
    );
  ...
  prove(options: {
    ...
    const { Pythia } = getPythiaModules();
    const myTransformedPassword = dataToUint8Array(
      options.transformedPassword,
      "base64"
    );
    const myBlindedPassword = dataToUint8Array(
      options.blindedPassword,
    ...
    const myTransformedTweak = dataToUint8Array(
      options.transformedTweak,
      "base64"
    );
  ...
  verify(options: {
    ...
    const myTweak = dataToUint8Array(options.tweak, "base64");
    const myTransformationPublicKey = dataToUint8Array(
      options.transformationPublicKey,
      "base64"
    );
  ...
  updateDeblindedWithToken(options: {
    ...
    const { Pythia } = getPythiaModules();
    const myDeblindedPassword = dataToUint8Array(
      options.deblindedPassword,
      "base64"
    );
    const myUpdateToken = dataToUint8Array(options.updateToken, "base64");
    const result = Pythia.updateDeblindedWithToken(
      myDeblindedPassword,
      myUpdateToken
    );
