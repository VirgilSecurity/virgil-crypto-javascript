...
describe('calculateHash', function () {
	it('should produce the same hash for the same data', function () {
		const hash1 = cryptoApi.hash(new Buffer('Hash me'));
		const hash2 = cryptoApi.hash(new Buffer('Hash me'));
	...
	it('should produce different hash for different algorithms', function () {
		const hash1 = cryptoApi.hash(new Buffer('Hash me'), HashAlgorithm.SHA256);
		const hash2 = cryptoApi.hash(new Buffer('Hash me'), HashAlgorithm.SHA384);
