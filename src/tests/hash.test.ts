...
describe('calculateHash', function () {
	it('should produce the same hash for the same data', function () {
		const hash1 = cryptoApi.hash(Buffer.from('Hash me'));
		const hash2 = cryptoApi.hash(Buffer.from('Hash me'));
	...
	it('should produce different hash for different algorithms', function () {
		const hash1 = cryptoApi.hash(Buffer.from('Hash me'), HashAlgorithm.SHA256);
		const hash2 = cryptoApi.hash(Buffer.from('Hash me'), HashAlgorithm.SHA384);
